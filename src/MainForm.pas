...
type
  TfrmMainForm = class(TForm)
    ...
    procedure btcascadeClick(Sender: TObject);
  private
...
procedure TfrmMainForm.btcascadeClick(Sender: TObject);
begin
  Cascade;
end;
...
procedure TfrmMainForm.Deshabilita;
begin
  btinsert.enabled := false;
  btedit.enabled := btinsert.enabled;
  btdelete.enabled := btinsert.enabled;
  btpost.enabled :=  btinsert.enabled;
  btcancel.enabled := btinsert.enabled;
end;
...
procedure TfrmMainForm.R1Click(Sender: TObject);
begin
  Application.CreateForm(TfrmReportRentDevol, frmReportRentDevol);
  frmReportRentDevol.Show;
end;
